...
import { adminTrips, adminLocations } from "@/lib/api";
...
export default function AdminTripsPage() {
  ...
  const [form, setForm] = useState({ title: "", description: "", start_date: "", end_date: "", location_id: "" });
    ...
    <div className="p-8">
      ...
      {creating && (
        <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 mb-4 space-y-3">
          ...
          <div className="flex gap-2">
            <button onClick={() => form.title && create.mutate()} disabled={!form.title || create.isPending}
              className="bg-brand-600 hover:bg-brand-700 disabled:opacity-50 text-white text-sm px-4 py-2 rounded transition-colors">
              Create
            </button>
            <button onClick={() => setCreating(false)} className="text-gray-400 text-sm px-3 py-2">Cancel</button>
          </div>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {trips?.map((trip) => (
            <div key={trip.id} className="bg-gray-900 border border-gray-800 rounded-lg px-4 py-3 flex items-center gap-4">
              <div className="flex-1">
                <p className="font-medium text-white">{trip.title}</p>
                {trip.start_date && (
                  <p className="text-xs text-gray-500">
                    {format(new Date(trip.start_date), "MMM d, yyyy")}
                    {trip.end_date && trip.end_date !== trip.start_date && ` — ${format(new Date(trip.end_date), "MMM d, yyyy")}`}
                  </p>
                )}
              </div>
              <label className="flex items-center gap-1.5 text-sm text-gray-400 cursor-pointer">
                <input type="checkbox" checked={trip.is_published}
                  onChange={(e) => togglePublish.mutate({ id: trip.id, val: e.target.checked })} className="rounded" />
                Published
              </label>
              <button onClick={() => { if (confirm(`Delete "${trip.title}"?`)) remove.mutate(trip.id); }}
